...
public class Save3D : MonoBehaviour {
    ...
    public void Save (RenderTexture volumeTexture, string saveName) {
        ...
        var x = Tex3DFromTex2DArray (slices, resolution,resolution);
        UnityEditor.AssetDatabase.CreateAsset (x, "Assets/Resources/" + saveName + ".asset");
#endif
    }
    
    public void Save2 (RenderTexture volumeTexture, string saveName) {
#if UNITY_EDITOR
        string sceneName = UnityEditor.SceneManagement.EditorSceneManager.GetActiveScene ().name;
        saveName = sceneName + "_" + saveName;
        Texture2D[] slices = new Texture2D[volumeTexture.volumeDepth];

        slicer.SetTexture (0, "volumeTexture", volumeTexture);

        for (int layer = 0; layer < volumeTexture.volumeDepth; layer++) {
            var slice = new RenderTexture (volumeTexture.width, volumeTexture.height, 0);
            slice.dimension = UnityEngine.Rendering.TextureDimension.Tex2D;
            slice.enableRandomWrite = true;
            slice.Create ();
            ...
            slicer.SetTexture (0, "slice", slice);
            slicer.SetInt ("layer", layer);
            int numThreadGroups = Mathf.CeilToInt (volumeTexture.width / (float) threadGroupSize);
            slicer.Dispatch (0, numThreadGroups, numThreadGroups, 1);
        ...
        var x = Tex3DFromTex2DArray (slices,volumeTexture.width, volumeTexture.volumeDepth);
        UnityEditor.AssetDatabase.CreateAsset (x, "Assets/Resources/" + saveName + ".asset");
#endif
    }

    Texture3D Tex3DFromTex2DArray (Texture2D[] slices, int resolution,int depth) {
        Texture3D tex3D = new Texture3D (resolution, resolution, depth, TextureFormat.ARGB32, false);
