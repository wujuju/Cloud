...
#define UIF (1./float(4294967295u))
...
float gradientNoise(float3 x, float freq)
{
    ...
    // gradients
    float3 ga = hash33(glsl_mod(p + float3(0., 0., 0.), freq));
    float3 gb = hash33(glsl_mod(p + float3(1., 0., 0.), freq));
    float3 gc = hash33(glsl_mod(p + float3(0., 1., 0.), freq));
    float3 gd = hash33(glsl_mod(p + float3(1., 1., 0.), freq));
    float3 ge = hash33(glsl_mod(p + float3(0., 0., 1.), freq));
    float3 gf = hash33(glsl_mod(p + float3(1., 0., 1.), freq));
    float3 gg = hash33(glsl_mod(p + float3(0., 1., 1.), freq));
    float3 gh = hash33(glsl_mod(p + float3(1., 1., 1.), freq));
    ...
    for (float x = -1.; x <= 1.; ++x)
    {
        for (float y = -1.; y <= 1.; ++y)
        {
            for (float z = -1.; z <= 1.; ++z)
            {
                ...
                float3 h = hash33(glsl_mod(id + offset, (float3)freq)) * .5 + .5;
                h += offset;
                float3 d = p - h;
